 ...
 */
	...
	for( i=0; i<rows; i++ )
		for( j=0; j<columns; j++ ) {
			accessMat( surface, i, j ) = 0.0; accessMat( surfaceCopy, i, j ) = 0.0;
		}
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		for( i=0; i<num_focal; i++ ) { if ( focal[i].start == iter ) { focal[i].active = 1; if ( ! first_activation ) first_activation = 1; } if ( focal[i].active == 2 ) num_deactivated++; }
		...
		for( step=0; step<10; step++ )	{
			...
			//#pragma omp parallel for firstprivate(surfaceCopy, surface, rows, columns)
			for( i=1; i<rows-1; i++ ) /* 4.2.3. Update surface values (skip borders) */
				for( j=1; j<columns-1; j++ )
					accessMat( surface, i, j ) = (accessMat( surfaceCopy, i-1, j ) + accessMat( surfaceCopy, i+1, j ) + accessMat( surfaceCopy, i, j-1 ) + accessMat( surfaceCopy, i, j+1 ) ) / 4;

			
			global_residual = 0.0f;/* 4.2.4. Compute the maximum residual difference (absolute value) */
			//#pragma omp parallel for reduction(max:global_residual) num_threads(2)
			for( i=1; i<rows-1; i++ )
				for( j=1; j<columns-1; j++ ) 
					if ( fabs( accessMat( surface, i, j ) - accessMat( surfaceCopy, i, j ) ) > global_residual ) { global_residual = fabs( accessMat( surface, i, j ) - accessMat( surfaceCopy, i, j ) ); }
		}
		...
		for( t=0; t<num_teams; t++ ) {
			...
			for( j=0; j<num_focal; j++ ) {
				if ( focal[j].active != 1 ){ continue;} float dx = focal[j].x - teams[t].x; float dy = focal[j].y - teams[t].y; float local_distance = sqrtf( dx*dx + dy*dy ); if ( local_distance < distance ) {	distance = local_distance;	target = j; }
			}
		...
		for( t=0; t<num_teams; t++ ) {
			...
			for( i=teams[t].x-radius; i<=teams[t].x+radius; i++ ) {
				for( j=teams[t].y-radius; j<=teams[t].y+radius; j++ ) {
					if ( i<1 || i>=rows-1 || j<1 || j>=columns-1 ){ continue;} float dx = teams[t].x - i; float dy = teams[t].y - j; float distance = sqrtf( dx*dx + dy*dy ); if ( distance <= radius ) { accessMat( surface, i, j ) = accessMat( surface, i, j ) * ( 1 - 0.25 );}
				}
			}
		}
