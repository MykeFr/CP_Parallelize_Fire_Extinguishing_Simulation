 ...
 */
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		for( step=0; step<10; step++ )	{
			...
			//#pragma omp parallel for shared(surfaceCopy, surface, rows, columns) private(i, j) reduction(max:global_residual) num_threads(num_threads)
			for( i=1; i<rows-1; i++ )
				for( j=1; j<columns-1; j++ ) 
		...
		for (t = 0; t < num_teams; t++) {
			...
			for( j=0; j<num_focal; j++ ) {
				...
				if (local_distance  < distance * distance ) {
